...
class index extends foreground {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : '';
					if($status == -1) {	//用户不存在
						showmessage(L('user_not_exist'), $forward);
					} elseif($status == -2) { //密码错误
						showmessage(L('password_error'), $forward);
					} else {
						showmessage(L('login_failure'), $forward);
					}
				}
	...
	public function mini() {
		...
		$grouplist = getcache('grouplist');
		$_groupname = $grouplist[$_groupid]['name'];
